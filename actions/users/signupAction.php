...
if (isset($_POST['validate'])) {
    ...
    if (!empty($_POST['pseudo']) && !empty($_POST['lastname']) && !empty($_POST['firstname']) && !empty($_POST['password'])) {
        ...
        if ($checkIfUserAlreadyExists->rowCount() == 0) {
            ...
            $insertUserOnWebsite->bindParam(1, $user_pseudo);
            $insertUserOnWebsite->bindParam(2, $user_lastname);
            $insertUserOnWebsite->bindParam(3, $user_firstname);
            $insertUserOnWebsite->bindParam(4, $user_password);
            ...
            $getInfosOfThisUserReq->bindParam(1, $user_lastname);
            $getInfosOfThisUserReq->bindParam(2, $user_firstname);
            $getInfosOfThisUserReq->bindParam(3, $user_pseudo);
            $getInfosOfThisUserReq->execute();

            $usersInfos = $getInfosOfThisUserReq->fetch();

            // Authentifier l'utilisateur sur le site et récupérer ses données dans la variables globales sessions
            $_SESSION['auth'] = true;
